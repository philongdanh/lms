---
id: faq
title: FAQ
sidebar_label: FAQ
---

# Frequently Asked Questions

Tổng hợp các câu hỏi thường gặp và giải đáp về LMS Platform.


## General

### LMS Platform là gì?

LMS Platform là hệ thống quản lý học tập thông minh, kết hợp **AI Learning Paths** và **Gamification** để cá nhân hóa trải nghiệm học tập cho học sinh từ lớp 1-12.

### LMS dành cho ai?

| Audience | Description |
|----------|-------------|
| **Học sinh** | Lớp 1-12, học tập cá nhân hóa và thi đấu |
| **Phụ huynh** | Theo dõi tiến độ và báo cáo học tập |
| **Giáo viên** | Quản lý lớp học, tạo đề thi, giám sát học sinh |
| **Trường học** | Quản trị hệ thống, tổ chức tournament |

### Chi phí sử dụng như thế nào?

LMS được cung cấp thông qua trường học. Vui lòng liên hệ quản trị viên trường để biết chi tiết.


## Account

### Làm thế nào để tạo tài khoản?

1. Liên hệ quản trị viên trường học để được cấp tài khoản
2. Hoặc đăng ký tại trang chủ LMS (nếu được cho phép)
3. Xác nhận email và đăng nhập lần đầu

### Làm thế nào để đặt lại mật khẩu?

1. Nhấn **Quên mật khẩu** tại trang đăng nhập
2. Nhập email đã đăng ký
3. Kiểm tra email và làm theo hướng dẫn

### Làm thế nào để xóa tài khoản?

Liên hệ quản trị viên trường học hoặc gửi yêu cầu qua email hỗ trợ.


## Features

### Làm thế nào để tham gia Tournament?

1. Vào menu **Tournament** trong thanh điều hướng
2. Chọn cuộc thi có trạng thái **Đang mở đăng ký**
3. Nhấn **Tham gia** và chờ đến giờ thi
4. Hoàn thành bài thi trong thời gian quy định

### Làm thế nào để xem Learning Path?

1. Đăng nhập vào hệ thống
2. Vào **Dashboard** hoặc **Lộ trình học tập**
3. Hệ thống AI sẽ gợi ý lộ trình phù hợp với năng lực của bạn

### Hệ thống điểm và huy hiệu (Gamification) hoạt động như thế nào?

- **Điểm kinh nghiệm (XP)**: Nhận được khi hoàn thành bài học, bài tập
- **Huy hiệu (Badge)**: Đạt được khi hoàn thành mục tiêu đặc biệt
- **Chuỗi ngày (Streak)**: Duy trì học liên tục để tăng streak
- **Bảng xếp hạng**: So sánh với bạn bè và các học sinh khác


## Troubleshooting

### Tại sao tôi không thể đăng nhập?

**Nguyên nhân có thể**:
- Sai email hoặc mật khẩu
- Tài khoản chưa được kích hoạt
- Tài khoản bị khóa

**Giải pháp**:
1. Kiểm tra lại thông tin đăng nhập
2. Thử đặt lại mật khẩu
3. Liên hệ quản trị viên nếu vẫn không được

### Tại sao tôi không thể nộp bài thi?

**Nguyên nhân có thể**:
- Mất kết nối internet
- Hết thời gian làm bài
- Lỗi trình duyệt

**Giải pháp**:
1. Kiểm tra kết nối internet
2. Refresh trang và thử lại
3. Báo lỗi cho quản trị viên để được xem xét

### Làm thế nào để báo cáo lỗi?

1. Truy cập [GitHub Issues](https://github.com/idlongggg/lms/issues)
2. Nhấn **New Issue**
3. Mô tả chi tiết lỗi (các bước, ảnh chụp màn hình)


## Technical

### Những trình duyệt nào được hỗ trợ?

| Browser | Minimum Version |
|---------|-----------------|
| Chrome | 90+ |
| Firefox | 88+ |
| Safari | 14+ |
| Edge | 90+ |

### Yêu cầu hệ thống là gì?

| Requirement | Specification |
|-------------|---------------|
| Kết nối internet | Tối thiểu 1 Mbps |
| Màn hình | Tối thiểu 1024x768 |
| JavaScript | Bật |
| Cookies | Bật |

---

## Still Have Questions?

- **Email hỗ trợ**: Liên hệ quản trị viên trường học
- **Tài liệu kỹ thuật**: [Developer Guide](/developer-guide/setup)
- **GitHub**: [LMS Repository](https://github.com/idlongggg/lms)
