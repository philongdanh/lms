...
Mô hình dữ liệu để lưu trữ thông tin thành tích, điểm số và phần thưởng của
người dùng.
...
- **Concurrency**: Sử dụng Row Locking hoặc Optimistic Locking (cột `version`)
  cho `UserExp` để tránh race conditions khi cộng điểm.

---

## Performance Requirements

- **Write**: Việc cộng điểm/xu được xử lý bất đồng bộ qua Queue để đảm bảo thời
  gian phản hồi thấp cho luồng Learning.
...
- **Integrity**: Không cho phép sửa đổi trực tiếp `exp_points` từ API client
  (chỉ thông qua server-side events).
