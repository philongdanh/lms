...
| **Cache & Pub/Sub** | Redis | 7+ | Session cache, real-time Pub/Sub, leaderboard | Độ trễ thấp (TC-PERF-05) |
...
| **Real-time** | Socket.IO Server + Redis adapter | Room-based, auto-reconnect, scaling với Redis | WebSocket scaling (TC-PERF-05) |
| **Authentication** | Passport.js + JWT | Linh hoạt strategy, hỗ trợ multi-tenant | RBAC 5 roles (TC-SEC-01) |
...
| **Cache & Pub/Sub** | Redis 7+ Cluster | Cache, session storage, real-time messaging | Real-time events (TC-PERF-06) |
...
| **Load Balancer** | Nginx với WebSocket support | Reverse proxy, load balancing, SSL termination | WebSocket support (TC-PERF-05) |
...
| **Authorization** | RBAC với 5 roles | Phân quyền chi tiết | RBAC (TC-SEC-01) |
