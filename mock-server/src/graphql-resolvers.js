...
export const queryResolvers = {
  ...
  topics: (_, { subjectId }) =>
    subjectId ? db.topics.filter((t) => t.subjectId === subjectId) : db.topics,
  lessons: (_, { topicId }) =>
    topicId ? db.lessons.filter((l) => l.topicId === topicId) : db.lessons,
  ...
  badges: () =>
    db.badges.slice(0, 3).map((b) => ({
      userId: currentUser.id,
      badgeId: b.id,
      badge: b,
      earnedAt: new Date().toISOString(),
    })),
  leaderboard: (_, { limit = 10 }) =>
    db.users.slice(0, limit).map((u, i) => ({
      rank: i + 1,
      userId: u.id,
      user: u,
      score: 10000 - i * 500,
  ...
  // Tournament
  tournaments: (_, { status }) =>
    status ? db.tournaments.filter((t) => t.status === status) : db.tournaments,
  ...
  tournamentLeaderboard: (_, { tournamentId }) =>
    db.users
      .slice(0, 10)
      .map((u, i) => generateParticipant(tournamentId, u.id)),
...
export const mutationResolvers = {
  ...
  register: (_, { input }) =>
    generateUser({ email: input.email, status: 'PENDING' }),
  refreshToken: () => ({
    accessToken: 'mock-access-token-' + uuid(),
    refreshToken: 'mock-refresh-token-' + uuid(),
    user: currentUser,
  ...
  // Learning
  completeLesson: (_, { id }) =>
    generateLessonProgress(currentUser.id, id, { status: 'COMPLETED' }),
  ...
  joinTournament: (_, { tournamentId }) =>
    generateParticipant(tournamentId, currentUser.id),
...
export const typeResolvers = {
  User: {
    ...
    roles: (user) =>
      user.roles || [{ id: uuid(), userId: user.id, role: 'STUDENT' }],
    sessions: () => [],
